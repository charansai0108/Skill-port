(function () {
  ...
  // ✅ Extract challenge details with robust difficulty detection
  ...
  function getDifficulty() {
    console.log('🔍 HackerRank: Starting difficulty extraction...');
    
    // Try multiple selectors for difficulty
    const selectors = [
      '[data-difficulty]', // Data attribute
      '.difficulty', '.challenge-difficulty', '[data-testid="difficulty"]', // Common selectors
      ...
      '.challenge-difficulty-badge', '.difficulty-badge' // HackerRank specific
    ];
    ...
    console.log(`🔍 HackerRank: Found ${allDifficultyElements.length} difficulty-related elements`);
    ...
    console.log(`🔍 HackerRank: Found ${difficultyTextElements.length} elements with difficulty text:`, difficultyTextElements);
    
    // Use the difficulty text elements we found
    if (difficultyTextElements.length > 0) {
      const firstElement = difficultyTextElements[0];
      console.log(`✅ HackerRank: Difficulty extracted from text element: ${firstElement.text}`);
    ...
    for (const element of allDifficultyElements) {
      ...
      console.log(`🔍 HackerRank: Checking element with classes: ${classes}, text: "${text}"`);
      
      // Check CSS classes
      if (classes.includes('difficulty-easy') || classes.includes('text-difficulty-easy')) {
        console.log('✅ HackerRank: Difficulty extracted from CSS class: easy');
        return 'easy';
      }
      if (classes.includes('difficulty-medium') || classes.includes('text-difficulty-medium')) {
        console.log('✅ HackerRank: Difficulty extracted from CSS class: medium');
        return 'medium';
      }
      if (classes.includes('difficulty-hard') || classes.includes('text-difficulty-hard')) {
        console.log('✅ HackerRank: Difficulty extracted from CSS class: hard');
        return 'hard';
      }
      ...
      if (dataDiff) {
        ...
        if (['easy', 'medium', 'hard'].includes(diff)) {
          console.log('✅ HackerRank: Difficulty extracted from data attribute:', diff);
          return diff;
        }
      }
      ...
      if (['easy', 'medium', 'hard'].includes(text)) {
        console.log('✅ HackerRank: Difficulty extracted from text:', text);
        return text;
      }
    }
    ...
    console.log('⚠️ HackerRank: Difficulty not found, defaulting to medium');
    return 'medium';
  }
