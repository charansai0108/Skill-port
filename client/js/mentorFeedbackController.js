import EnhancedPageController from './enhancedPageController.js';

class MentorFeedbackController extends EnhancedPageController {
    constructor() {
        super();
        this.realTimeListeners = [];
    }

    getRequiredRole() {
        return 'mentor';
    }

    async renderDashboardContent() {
        console.log('💬 MentorFeedbackController: Rendering feedback management content...');
        
        try {
            await this.renderFeedbackList();
            await this.renderFeedbackStats();
            await this.renderPendingFeedback();
            await this.renderFeedbackHistory();
            
            this.setupFormHandlers();
            this.setupRealTimeListeners();
            
            console.log('✅ MentorFeedbackController: Feedback management content rendered successfully');
            
        } catch (error) {
            console.error('❌ MentorFeedbackController: Error rendering feedback content:', error);
            throw error;
        }
    }

    async renderFeedbackList() {
        try {
            const feedback = await this.dataLoader.loadMentorFeedback(100);
            this.updateFeedbackListUI(feedback);
        } catch (error) {
            console.error('Error loading feedback list:', error);
            this.showDefaultFeedbackList();
        }
    }

    async renderFeedbackStats() {
        try {
            const stats = await this.dataLoader.loadFeedbackStats();
            this.updateFeedbackStatsUI(stats);
        } catch (error) {
            console.error('Error loading feedback stats:', error);
            this.showDefaultFeedbackStats();
        }
    }

    async renderPendingFeedback() {
        try {
            const pending = await this.dataLoader.loadPendingFeedback(50);
            this.updatePendingFeedbackUI(pending);
        } catch (error) {
            console.error('Error loading pending feedback:', error);
            this.showDefaultPendingFeedback();
        }
    }

    async renderFeedbackHistory() {
        try {
            const history = await this.dataLoader.loadFeedbackHistory(50);
            this.updateFeedbackHistoryUI(history);
        } catch (error) {
            console.error('Error loading feedback history:', error);
            this.showDefaultFeedbackHistory();
        }
    }

    updateFeedbackListUI(feedback) {
        const container = document.getElementById('feedback-list');
        if (!container) return;

        if (feedback.length === 0) {
            container.innerHTML = `
                <div class="text-center py-8">
                    <i data-lucide="message-square" class="w-12 h-12 text-gray-400 mx-auto mb-4"></i>
                    <p class="text-gray-500">No feedback received</p>
                </div>
            `;
            return;
        }

        container.innerHTML = feedback.map(item => `
            <div class="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-sm transition-shadow">
                <div class="flex items-start space-x-3 mb-3">
                    <img src="${item.student?.profileImage || '/images/default-avatar.png'}" 
                         alt="${item.student?.name}" class="w-10 h-10 rounded-full object-cover">
                    <div class="flex-1">
                        <div class="flex items-center justify-between mb-1">
                            <h3 class="text-lg font-semibold text-gray-900">${item.student?.name || 'Anonymous'}</h3>
                            <div class="flex items-center space-x-2">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${this.getRatingColor(item.rating)}">
                                    ⭐ ${item.rating || 'N/A'}
                                </span>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${this.getStatusColor(item.status)}">
                                    ${item.status}
                                </span>
                            </div>
                        </div>
                        <p class="text-sm text-gray-600">${item.session?.title || 'Session Feedback'}</p>
                        <p class="text-xs text-gray-500">${this.formatDateTime(item.createdAt)}</p>
                    </div>
                </div>
                
                <div class="mb-3">
                    <p class="text-sm text-gray-700">${item.comment || 'No comment provided'}</p>
                </div>

                <div class="grid grid-cols-2 gap-4 mb-3 text-sm">
                    <div>
                        <span class="text-gray-500">Session:</span>
                        <span class="font-medium text-gray-900 ml-2">${item.session?.title || 'N/A'}</span>
                    </div>
                    <div>
                        <span class="text-gray-500">Duration:</span>
                        <span class="font-medium text-gray-900 ml-2">${item.session?.duration || 'N/A'}</span>
                    </div>
                    <div>
                        <span class="text-gray-500">Helpfulness:</span>
                        <span class="font-medium text-gray-900 ml-2">${item.helpfulness || 'N/A'}</span>
                    </div>
                    <div>
                        <span class="text-gray-500">Communication:</span>
                        <span class="font-medium text-gray-900 ml-2">${item.communication || 'N/A'}</span>
                    </div>
                </div>

                <div class="flex items-center justify-between">
                    <div class="text-xs text-gray-500">
                        ID: ${item.id}
                    </div>
                    <div class="flex space-x-2">
                        <button class="text-blue-600 hover:text-blue-700 text-sm font-medium" onclick="viewFeedback('${item.id}')">
                            View
                        </button>
                        <button class="text-green-600 hover:text-green-700 text-sm font-medium" onclick="replyToFeedback('${item.id}')">
                            Reply
                        </button>
                        <button class="text-purple-600 hover:text-purple-700 text-sm font-medium" onclick="flagFeedback('${item.id}')">
                            Flag
                        </button>
                    </div>
                </div>
            </div>
        `).join('');
    }

    updateFeedbackStatsUI(stats) {
        const statsElements = {
            'total-feedback': stats.totalFeedback || 0,
            'average-rating': stats.averageRating || 'N/A',
            'positive-feedback': stats.positiveFeedback || 0,
            'negative-feedback': stats.negativeFeedback || 0,
            'pending-replies': stats.pendingReplies || 0,
            'this-month': stats.thisMonth || 0,
            'this-week': stats.thisWeek || 0,
            'today': stats.today || 0
        };

        Object.entries(statsElements).forEach(([id, value]) => {
            this.updateElement(id, value);
        });
    }

    updatePendingFeedbackUI(pending) {
        const container = document.getElementById('pending-feedback-list');
        if (!container) return;

        if (pending.length === 0) {
            container.innerHTML = `
                <div class="text-center py-8">
                    <i data-lucide="clock" class="w-12 h-12 text-gray-400 mx-auto mb-4"></i>
                    <p class="text-gray-500">No pending feedback</p>
                </div>
            `;
            return;
        }

        container.innerHTML = pending.map(item => `
            <div class="flex items-start space-x-3 p-3 bg-white rounded-lg border border-gray-200">
                <div class="flex-shrink-0">
                    <div class="w-8 h-8 rounded-full bg-yellow-100 flex items-center justify-center">
                        <i data-lucide="clock" class="w-4 h-4 text-yellow-600"></i>
                    </div>
                </div>
                <div class="flex-1">
                    <div class="flex items-center justify-between mb-1">
                        <h4 class="text-sm font-medium text-gray-900">${item.student?.name || 'Anonymous'}</h4>
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                            Pending
                        </span>
                    </div>
                    <p class="text-xs text-gray-500">${item.session?.title || 'Session Feedback'}</p>
                    <p class="text-xs text-gray-500 mt-1">${this.formatDateTime(item.createdAt)}</p>
                </div>
                <div class="flex space-x-2">
                    <button class="text-green-600 hover:text-green-700 text-sm font-medium" onclick="replyToFeedback('${item.id}')">
                        Reply
                    </button>
                    <button class="text-blue-600 hover:text-blue-700 text-sm font-medium" onclick="viewFeedback('${item.id}')">
                        View
                    </button>
                </div>
            </div>
        `).join('');

        if (window.lucide) window.lucide.createIcons();
    }

    updateFeedbackHistoryUI(history) {
        const container = document.getElementById('feedback-history-list');
        if (!container) return;

        if (history.length === 0) {
            container.innerHTML = `
                <div class="text-center py-8">
                    <i data-lucide="history" class="w-12 h-12 text-gray-400 mx-auto mb-4"></i>
                    <p class="text-gray-500">No feedback history</p>
                </div>
            `;
            return;
        }

        container.innerHTML = history.map(item => `
            <div class="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200">
                <div class="flex items-center space-x-3">
                    <img src="${item.student?.profileImage || '/images/default-avatar.png'}" 
                         alt="${item.student?.name}" class="w-8 h-8 rounded-full object-cover">
                    <div>
                        <p class="text-sm font-medium text-gray-900">${item.student?.name || 'Anonymous'}</p>
                        <p class="text-xs text-gray-500">${item.session?.title || 'Session Feedback'}</p>
                    </div>
                </div>
                <div class="text-right">
                    <div class="text-sm font-medium text-gray-900">⭐ ${item.rating || 'N/A'}</div>
                    <div class="text-xs text-gray-500">${this.formatDate(item.createdAt)}</div>
                </div>
                <div class="text-right">
                    <div class="text-sm font-medium text-gray-900">${item.status}</div>
                    <div class="text-xs text-gray-500">status</div>
                </div>
            </div>
        `).join('');
    }

    setupFormHandlers() {
        // Feedback filter
        const filterSelect = document.getElementById('feedback-filter');
        if (filterSelect) {
            filterSelect.addEventListener('change', (e) => {
                this.filterFeedback(e.target.value);
            });
        }

        // Search feedback
        const searchInput = document.getElementById('feedback-search');
        if (searchInput) {
            searchInput.addEventListener('input', (e) => {
                this.searchFeedback(e.target.value);
            });
        }

        // Reply form
        const replyForm = document.getElementById('feedback-reply-form');
        if (replyForm) {
            replyForm.addEventListener('submit', (e) => {
                e.preventDefault();
                this.replyToFeedback();
            });
        }
    }

    async filterFeedback(filter) {
        try {
            const feedback = await this.dataLoader.loadMentorFeedback(100);
            let filteredFeedback = feedback;

            if (filter !== 'all') {
                filteredFeedback = feedback.filter(item => item.status === filter);
            }

            this.updateFeedbackListUI(filteredFeedback);
            
        } catch (error) {
            console.error('Error filtering feedback:', error);
        }
    }

    async searchFeedback(query) {
        try {
            const feedback = await this.dataLoader.loadMentorFeedback(100);
            let filteredFeedback = feedback;

            if (query.trim()) {
                filteredFeedback = feedback.filter(item => 
                    item.student?.name?.toLowerCase().includes(query.toLowerCase()) ||
                    item.comment?.toLowerCase().includes(query.toLowerCase()) ||
                    item.session?.title?.toLowerCase().includes(query.toLowerCase())
                );
            }

            this.updateFeedbackListUI(filteredFeedback);
            
        } catch (error) {
            console.error('Error searching feedback:', error);
        }
    }

    async replyToFeedback() {
        try {
            const formData = new FormData(document.getElementById('feedback-reply-form'));
            const feedbackId = formData.get('feedbackId');
            const reply = formData.get('reply');

            await this.firebaseService.replyToFeedback(feedbackId, reply);
            this.showSuccessMessage('Reply sent successfully!');
            document.getElementById('feedback-reply-form').reset();
            this.renderPendingFeedback(); // Refresh pending feedback
            
        } catch (error) {
            console.error('Error replying to feedback:', error);
            this.showErrorMessage('Failed to send reply. Please try again.');
        }
    }

    getRatingColor(rating) {
        if (rating >= 4.5) return 'bg-green-100 text-green-800';
        if (rating >= 3.5) return 'bg-yellow-100 text-yellow-800';
        if (rating >= 2.5) return 'bg-orange-100 text-orange-800';
        return 'bg-red-100 text-red-800';
    }

    getStatusColor(status) {
        const colors = {
            'pending': 'bg-yellow-100 text-yellow-800',
            'replied': 'bg-green-100 text-green-800',
            'flagged': 'bg-red-100 text-red-800',
            'resolved': 'bg-blue-100 text-blue-800'
        };
        return colors[status?.toLowerCase()] || 'bg-gray-100 text-gray-800';
    }

    showSuccessMessage(message) {
        this.showMessage(message, 'success');
    }

    showErrorMessage(message) {
        this.showMessage(message, 'error');
    }

    showMessage(message, type) {
        const messageDiv = document.createElement('div');
        messageDiv.className = `fixed top-4 right-4 p-4 rounded-lg shadow-lg z-50 ${
            type === 'success' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
        }`;
        messageDiv.textContent = message;
        
        document.body.appendChild(messageDiv);
        
        setTimeout(() => {
            document.body.removeChild(messageDiv);
        }, 3000);
    }

    // Default states
    showDefaultFeedbackList() {
        const container = document.getElementById('feedback-list');
        if (container) {
            container.innerHTML = `
                <div class="text-center py-8">
                    <i data-lucide="loader" class="w-12 h-12 text-gray-400 mx-auto mb-4 animate-spin"></i>
                    <p class="text-gray-500">Loading feedback...</p>
                </div>
            `;
        }
    }

    showDefaultFeedbackStats() {
        const defaultStats = {
            totalFeedback: 0,
            averageRating: 'N/A',
            positiveFeedback: 0,
            negativeFeedback: 0,
            pendingReplies: 0,
            thisMonth: 0,
            thisWeek: 0,
            today: 0
        };
        this.updateFeedbackStatsUI(defaultStats);
    }

    showDefaultPendingFeedback() {
        const container = document.getElementById('pending-feedback-list');
        if (container) {
            container.innerHTML = `
                <div class="text-center py-8">
                    <i data-lucide="loader" class="w-12 h-12 text-gray-400 mx-auto mb-4 animate-spin"></i>
                    <p class="text-gray-500">Loading pending feedback...</p>
                </div>
            `;
        }
    }

    showDefaultFeedbackHistory() {
        const container = document.getElementById('feedback-history-list');
        if (container) {
            container.innerHTML = `
                <div class="text-center py-8">
                    <i data-lucide="loader" class="w-12 h-12 text-gray-400 mx-auto mb-4 animate-spin"></i>
                    <p class="text-gray-500">Loading feedback history...</p>
                </div>
            `;
        }
    }

    setupRealTimeListeners() {
        console.log('💬 MentorFeedbackController: Setting up real-time listeners...');
        
        try {
            const feedbackListener = this.dataLoader.setupMentorFeedbackListener((feedback) => {
                console.log('💬 Feedback updated:', feedback);
                this.updateFeedbackListUI(feedback);
            });
            this.realTimeListeners.push(feedbackListener);

            console.log('✅ MentorFeedbackController: Real-time listeners setup completed');
            
        } catch (error) {
            console.error('❌ MentorFeedbackController: Error setting up real-time listeners:', error);
        }
    }

    destroy() {
        console.log('💬 MentorFeedbackController: Cleaning up...');
        this.realTimeListeners.forEach(unsubscribe => unsubscribe());
        this.realTimeListeners = [];
    }
}

document.addEventListener('DOMContentLoaded', () => {
    new MentorFeedbackController();
});

export default MentorFeedbackController;
