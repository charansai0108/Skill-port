...
export default function CommunityMentorsPage() {
  ...
  const fetchMentors = async () => {
    try {
      ...
      const response = await fetch(`/api/community/${communitySlug}/mentors?${params}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      // Calculate stats
      const activeMentors = mentorsData.filter((m: any) => m.status === 'active').length
      const inactiveMentors = mentorsData.filter((m: any) => m.status === 'inactive').length
      const avgRating = mentorsData.length > 0 
        ? mentorsData.reduce((sum: number, m: any) => sum + (m.rating || 0), 0) / mentorsData.length 
        : 0
      ...
      setStats({
        ...
        monthlyGrowth: {
          mentors: 0, // TODO: Calculate from historical data
          active: 0,
          inactive: 0,
          rating: 0
        }
  ...
  */
  ...
  const pageMentors = mentors // Pagination now handled by API
      ...
      <div className={`lg:hidden fixed inset-0 z-40 bg-white shadow-2xl transform transition-transform duration-200 ${
        ...
        <div className="p-6">
          <nav className="space-y-3">
            ...
            <Link href={`/community/${communitySlug}/mentors`} className="w-full text-left px-4 py-3 rounded-lg text-sm font-medium bg-red-50 text-red-700 border border-red-200 transition-all flex items-center gap-3">
              <GraduationCap className="w-4 h-4" />
              Mentors
            </Link>
          ...
          <AdminCard hover>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-green-600">+{stats.monthlyGrowth.mentors} this month</p>
              </div>
          ...
          <AdminCard hover>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{stats.inactiveMentors}</p>
