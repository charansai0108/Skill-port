...
export default function CommunityprofilePage() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const userResponse = await fetch(`/api/community/${communitySlug}/profile`, {
        headers: {
      ...
      setCommunityData(userResult.data.community)
    } catch (err: any) {
  ...
  const fetchBatches = async () => {
    try {
      ...
      const response = await fetch(`/api/community/${communitySlug}/batches`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  // Batches are now fetched dynamically from API
  ...
  const addStudent = async () => {
    if (!newStudentEmail || !selectedBatchId) {
    ...
    try {
      ...
      const response = await fetch(`/api/community/${communitySlug}/students`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          ...
          errorData = { error: 'Invalid response format', rawResponse: await response.text() }
        }
        ...
        showToast(errorData.error || `Failed to create batch (${response.status})`, 'error')
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 flex items-center justify-center">
        <div className="text-center">
          <div className="w-8 h-8 bg-gradient-to-br from-red-600 to-pink-600 rounded-lg flex items-center justify-center mx-auto mb-4 animate-pulse">
...
{communityData?.name || 'Loading...'}
            ...
            <div className="hidden md:flex items-center gap-1">
              ...
              <Link href={`/community/${communitySlug}/profile`} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-all ${isActive('/admin/profile') ? 'bg-red-50 text-red-700 shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">{userData?.name || authUser?.name || 'Admin'} Profile</h1>
              ...
              <div className="content-section bg-white rounded-2xl shadow-lg border border-slate-200/50 p-8">
                ...
                <div className="text-center mb-8">
                  <div className="profile-image-container relative inline-block mb-4">
                    <img
                      src={profileImage || userData?.profilePic || authUser?.profilePic || 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop&crop=face'}
                      alt="Admin Profile"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      type="text"
                      defaultValue={userData?.name || authUser?.name || ''}
                  ...
                  <div>
                    ...
                    <input
                      type="email"
                      defaultValue={userData?.email || authUser?.email || ''}
                      className="form-input w-full px-4 py-3 border border-slate-300 rounded-lg focus:outline-none transition-all"
                    />
                  </div>
                </div>
                    ...
                    {batches.map((batch, index) => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className={`text-xs px-2 py-1 rounded-full ${getColorClasses(['blue', 'green', 'purple', 'orange', 'red'][index % 5])}`}>
                            {batch._count?.students || batch.studentCount || 0} students
                          </span>
                        </div>
