...
export default function CommunitycontestsPage() {
  ...
  const fetchContests = async () => {
    try {
      ...
      const response = await fetch(`/api/community/${communitySlug}/contests?${params}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      const result = await response.json()
      const contestsData = result.data.contests || []
      setContests(contestsData)
      setStats(result.data.stats || {})
      
      // Calculate dynamic stats
      const activeContests = contestsData.filter((c: any) => c.status === 'active').length
      const completedContests = contestsData.filter((c: any) => c.status === 'completed').length
      const totalParticipants = contestsData.reduce((sum: number, c: any) => sum + (c.participants || 0), 0)
      ...
      setDynamicStats({
        ...
        totalParticipants,
        monthlyGrowth: {
          contests: 0, // TODO: Calculate from historical data
          active: 0,
          completed: 0,
          participants: 0
        }
      ...
      <div className={`lg:hidden fixed inset-0 z-40 bg-white shadow-2xl transform transition-transform duration-200 ${
        ...
        <div className="p-6">
          <nav className="space-y-3">
            ...
            <Link href={`/community/${communitySlug}/contests`} className="w-full text-left px-4 py-3 rounded-lg text-sm font-medium bg-red-50 text-red-700 border border-red-200 transition-all flex items-center gap-3">
              <Trophy className="w-4 h-4" />
              Contests
            </Link>
          ...
          <div className="stat-card bg-white rounded-lg shadow-lg p-6 border border-slate-200/50">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">{dynamicStats.activeContests}</p>
              </div>
          ...
          <div className="stat-card bg-white rounded-lg shadow-lg p-6 border border-slate-200/50">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-slate-900">{dynamicStats.totalParticipants}</p>
              </div>
