(function () {
  ...
  async function getUsername() {
    for (let i = 0; i < 10; i++) {
      ...
      // Try localStorage
      const storedUsername = localStorage.getItem('interviewbit_username') || 
  ...
  async function sendSubmissionData(data) {
    try {
      ...
      if (response && response.success) {
        console.log('✅ InterviewBit submission tracked successfully');
        showTrackingNotification('Submission tracked successfully!');
      } else {
        console.error('❌ Failed to track InterviewBit submission:', response?.error);
        showTrackingNotification('Failed to track submission', 'error');
      }
    } catch (error) {
      console.error('❌ Error sending InterviewBit submission data:', error);
  ...
  async function trackSubmission() {
    try {
      console.log('🔍 InterviewBit: Starting submission tracking...');
      ...
      if (!submissionDetails) {
        console.log('⚠️ InterviewBit: No submission details found');
        return;
      }
      ...
      console.log('📊 InterviewBit: Submission data:', submissionData);
      await sendSubmissionData(submissionData);
      
    } catch (error) {
      console.error('❌ InterviewBit: Error in trackSubmission:', error);
    }
  }
  ...
  console.log('🚀 InterviewBit content script loaded');
})();